...
const MovieCard = (props) => {
    ...
    return (
        <>
            {!loading &&<Link to={props.movieData.media_type === 'movie'
                    ? `/movie/${props.movieData.id}`
                    : `/tv/${props.movieData.id}`}
                    className="group relative w-full h-full flex-1 overflow-hidden rounded-2xl bg-black \">
                    <div className="relative w-full h-full flex-1 overflow-hidden rounded-2xl bg-black ">
                        <LazyLoadImage
