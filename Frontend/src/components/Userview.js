  ...
  return (
   ...
   <div class="h-[176px] flex flex-col justify-end items-center relative">
      ...
      <div class="w-full p-3 flex justify-evenly sm:justify-center gap-5 flex-wrap sm:gap-12 z-10 text-white font-medium text-sm md:text-base">
         <a class="tabs smoothie flex gap-1 items-center relative" href="/user?tab=history">
         ...
         <a class="false smoothie flex gap-1 items-center relative" href="/user?tab=watchlist">
   ...
   <div class="w-full my-2 px-2 max-w-6xl mx-auto smoothie min-h-screen">
      <div class="flex justify-between items-center my-4 mt-5">
         ...
         <select class="p-1 rounded-md bg-white text-black font-medium outline-none">
            <option value="recent">Recent </option>
            <option value="old">Oldest </option>
         </select>
      </div>
      <div class="flex flex-wrap w-full ">
         {movies?.slice(0).reverse().map((element) => {
          return (
            <a key={element.id} class="flex flex-col gap-2 p-1 lg:p-2 w-full sm:w-1/2 md:w-1/3 xl:w-1/4 sm:aspect-video mb-2 group" href={`"/${element.media_type}/watch/${element.id}"`}>
            <div class="flex-shrink-0 h-full w-full relative rounded-lg overflow-hidden shadow-md bg-[var(--light)]">
               ...
               <span class="absolute xl:opacity-0 group-hover:opacity-100 top-2 right-2 cursor-pointer z-50 bg-white/90 hover:bg-[#d83b44] hover:brightness-90 rounded-lg shadow-lg flex items-center justify-center smoothie p-0">
                  <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="p-[.45rem] shrink-0 size-8 hover:text-white text-black transition duration-75 ease-in">
                     <path d="M3 6h18"></path>
               ...
               <div class="absolute p-3 px-4 flex flex-col gap-1 z-20 w-full bottom-0 left-0 tracking-wide">
                  <div class="flex justify-between items-center flex-grow">
                     ...
                     <span class="flex-shrink-0 smoothie bg-whites/20 backdrop-blur-sm group-hover:bg-white/20 group-active:bg-[#00c1dbef] p-2 items-center flex justify-center rounded-full h-fit">
                        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="white" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-play">
                  ...
                  <div class="flex justify-end my-3 w-full"><button class="bg-white/10 rounded-lg text-white p-2 px-4">Clear</button></div>
      </div>
   </div>
</div>
  )
